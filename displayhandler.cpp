...
void DisplayHandler::printMidiMessage(const char* message) {
  ...
  String msg(message);
  // Espera-se que a mensagem contenha 5 linhas separadas por '\n'
  ...
  // Imprime as 4 primeiras linhas com fonte pequena (tamanho 1)
  tft.setTextSize(1);
  tft.setTextColor(TFT_WHITE);
  ...
  // Ajusta y para evitar que o texto ultrapasse a borda inferior
